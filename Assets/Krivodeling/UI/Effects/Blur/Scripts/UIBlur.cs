...
namespace Krivodeling.UI.Effects
{
    public class UIBlur : MonoBehaviour
    {
        ...
        public void EndBlur(float time)
        {
            ...
            StartCoroutine(EndBlurCoroutine());
        }
...
//#endif
        ...
        private IEnumerator EndBlurCoroutine()
        {
            var counter = TransitionStart;
            var IntensityStart = Intensity;
            while (Intensity > 0f)
            {
                counter -= Time.deltaTime;
                var lerpIndex = 1f - counter / TransitionStart;
                Intensity = Mathf.Lerp(IntensityStart, 0f, lerpIndex);
;               Color = Color.Lerp(StartColor, EndColor, lerpIndex);
